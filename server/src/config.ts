...
export function validateConfig() {
  ...
  if (!config.openaiApiKey) warnings.push('OPENAI_API_KEY not set - using fallback story generation');
  if (!config.mongoUri) warnings.push('MONGO_URI not set - persistence disabled');

  if (process.env.NODE_ENV === 'production') {
    if (config.jwtAccessSecret === 'dev_access_secret') errors.push('JWT_ACCESS_SECRET must be set in production');
    if (config.jwtRefreshSecret === 'dev_refresh_secret') errors.push('JWT_REFRESH_SECRET must be set in production');
  }
