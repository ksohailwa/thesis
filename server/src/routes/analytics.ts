...
router.get('/session/:id', requireAuth, requireRole('teacher'), async (req, res) => {
  ...
  const wordStats = Object.entries(byWord).map(([word, v]) => ({
    word,
    immediateAvg: v.immediate.length ? v.immediate.reduce((s, x) => s + x, 0) / v.immediate.length : 0,
  ...
  res.json({ wordStats, effortDist, audioStats, counts: { attempts: attempts.length, events: events.length, efforts: efforts.length } });
});
...
router.get('/session/:id/csv', requireAuth, requireRole('teacher'), async (req, res) => {
  ...
  if (type === 'events') {
    ...
    const rows = events.map(e => [e.ts.toISOString(), String(e.student), e.taskType, e.targetWord || '', e.type, JSON.stringify(e.payload || {})]);
    ...
    const attempts = await Attempt.find({ session: sessionId });
    const header = ['createdAt','student','taskType','targetWord','score','revealed','hintCount','attemptsCount','attempts'];
    const rows = attempts.map(a => [a.createdAt.toISOString(), String(a.student), a.taskType, a.targetWord, a.score, a.revealed, a.hintCount, a.attempts?.length || 0, (a.attempts||[]).map(x=>x.text).join('|')]);
    const csv = [header, ...rows].map(r => r.map(v => '"' + String(v).replace(/"/g,'""') + '"').join(',')).join('\n');
    res.setHeader('Content-Type', 'text/csv');
    res.setHeader('Content-Disposition', `attachment; filename="session_${sessionId}_attempts.csv"`);
    return res.send(csv);
  }
});
