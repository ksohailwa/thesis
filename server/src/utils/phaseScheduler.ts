...
export interface TargetOccurrence { word: string; paragraphIndex: number; sentenceIndex: number }

export interface PhasePlacement { phase: Phase; paragraphIndex?: number; sentenceIndex?: number }
...
export function scheduleWordPhases(word: string, occurrences: TargetOccurrence[], paragraphCount: number): Record<Phase, PhasePlacement> {
  ...
  for (const o of uniq) {
    if (picks.length === 0 || !picks.some(p => p.paragraphIndex === o.paragraphIndex && p.sentenceIndex === o.sentenceIndex)) {
      picks.push(o);
    }
  ...
  out.baseline = { phase: 'baseline', paragraphIndex: picks[0]?.paragraphIndex, sentenceIndex: picks[0]?.sentenceIndex };
  out.learning = { phase: 'learning', paragraphIndex: picks[1]?.paragraphIndex, sentenceIndex: picks[1]?.sentenceIndex };
  out.reinforcement = { phase: 'reinforcement', paragraphIndex: picks[2]?.paragraphIndex, sentenceIndex: picks[2]?.sentenceIndex };
  out.recall = { phase: 'recall' };
  return out;
}

export function buildPhaseSchedule(targetWords: string[], allOccurrences: TargetOccurrence[], paragraphCount: number): Record<string, Record<Phase, PhasePlacement>> {
