...
export async function getSmartRecommendations(
  ...
  for (const exp of pastExperiments) (exp.targetWords || []).forEach((w: string) => allWords.add(w));
  ...
  for (const word of allWords) {
    ...
    const avgDifficulty = allMetrics.reduce((sum, m) => sum + m.difficultyScore, 0) / allMetrics.length;
    const avgEngagement = 100 - allMetrics.reduce((sum, m) => sum + m.revealRate, 0) / allMetrics.length;
