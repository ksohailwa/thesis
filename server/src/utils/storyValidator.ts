...
export function validateStories(targetWords: string[], payload: StoriesPayload): ValidationResult {
  ...
  const checkStory = (name: 'A' | 'B', occ: Occurrence[]) => {
    // Count: exactly 4 per word (1st=prior, 2nd-3rd=learning, 4th=recall)
    for (const w of targetWords) {
      const c = countWord(occ, w);
      if (c !== 4) v.push(`Story ${name}: word "${w}" must appear exactly 4 times (got ${c}).`);
    }
    // Per-paragraph cap: no more than 2 occurrences of the same word in a paragraph
    ...
    for (const [key, count] of Object.entries(byParaWord)) {
      ...
      if (count > 2) {
        v.push(`Story ${name}: word "${word}" appears ${count} times in paragraph ${pIdx} (max 2).`);
      }
    }
